...
export function editTask(tasksDatabase, weekTask, newTask, driver, changeState, handleClose, taskEditIndex, originalWeek) {
  ...
  if (!conflict) {
    ...
    let message = 'There is a conflict. Are yo sure you would like to override the following tasks:'
...
export function checkConflicts(tasksDatabase, weekTask, newTask, driver) {
  ...
  let conflictIndex = []
  if (tasksDatabase[driver["id"]][weekTask]) {
    let conflictTasks = tasksDatabase[driver["id"]][weekTask].filter(function (task, index) {
      const isConflict = (task.day === newTask.day && ((task.start_time < newTask.end_time && task.start_time >= newTask.start_time) || (task.end_time > newTask.start_time && task.end_time <= newTask.end_time)))
      if (isConflict) {
...
function overrideTaskOnEdit(tasksDatabase, conflictIndex, taskEditIndex, driver, weekTask, changeState, newTask, originalWeek, sameTask) {
  ...
  if (sameTask) {
    ...
    if(originalWeek === weekTask){
      ...
      const filteredTasks = updatedTasksDatabase[driver["id"]][weekTask].filter((task, index) => { return (!conflictIndex.includes(index) && index!==taskEditIndex)})
      filteredTasks.push(newTask)
      updatedTasksDatabase[driver["id"]][weekTask] = filteredTasks;
      changeState(updatedTasksDatabase)
